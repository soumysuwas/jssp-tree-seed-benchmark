"""
Instance parsers. Two formats matter:
...
Run:  python reference/instance_reference.py
"""
from __future__ import annotations
import pathlib
import numpy as np
from decoder_reference import Instance


def load_taillard_original(path) -> list[Instance]:
    """Parse one tai{n}_{m}.txt file -> list of 10 Instances, in file order."""
    lines = [l.strip() for l in pathlib.Path(path).read_text().splitlines()]
    out, i = [], 0
    while i < len(lines):
...
# The paper (Table 3) uses exactly 40 instances. The HIGHEST IS ta75.
# the supplied data holds all 80 because it is the complete Taillard benchmark, but
# ta06-10, ta16-20, ta26-30, ta36-40, ta46-50, ta56-60, ta66-70 and ta76-80 are
# NOT in the paper -- there is no Table 5 row to compare them against. Do not run
# them: ta76-80 are 100x20 and would nearly double the most expensive group
# (ta71-75 is already ~73% of total compute) for zero comparable output.
PAPER_INSTANCES = [f"ta{k:02d}" for base in (1, 11, 21, 31, 41, 51, 61, 71)
                   for k in range(base, base + 5)]
assert len(PAPER_INSTANCES) == 40 and PAPER_INSTANCES[-1] == "ta75"

NOT_IN_PAPER = sorted({f"ta{k:02d}" for k in range(1, 81)} - set(PAPER_INSTANCES))
...
if __name__ == "__main__":
    ...
    print("paper's 40 instances:", " ".join(PAPER_INSTANCES))
    p = pathlib.Path("data/raw/tai15_15.txt")
    if p.exists():
        insts = load_taillard_original(p)
        print(f"parsed {len(insts)} instances from {p}")
        i0 = insts[0]
        print(f"  ta01: n={i0.n} m={i0.m} D={i0.D} UB={i0.ub} LB={i0.lb}")
        print(f"  job1 first 3 ops -> machines {(i0.route[0][:3]+1).tolist()} "
              f"times {i0.ptime[0][:3].tolist()}   (expect [7,13,5] / [94,66,10])")
    else:
        print(f"{p} not found — run scripts/fetch_data.py first")